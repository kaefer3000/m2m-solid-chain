...
/**
 * Returns list of resources of available money transfers in inbox container
 * @param {string} inboxURL url of inbox container
 * @param {function} fetch
 */
export const fetchResourceInfoFromInbox = async (
  inboxURL,
  fetch
) => {
  const moneyTransfers = await fetchURLsFromContainer(inboxURL, fetch);

  const resources = await Promise.all(
    moneyTransfers.map(async (moneyTransfer) => {
      const moneyTransferStore = await requestRDF(moneyTransfer, fetch);
	  var resourceURLFromMoneyTransfer;
	  var amountFromMoneyTransfer;
	  var objectFromMoneyTransfer;
	  try {
		  const resourceURL = moneyTransferStore.getSubjects(
			null,
			"http://schema.org/MoneyTransfer"
		  );
		  resourceURLFromMoneyTransfer = moneyTransfer + resourceURL[0].id;
		  
		  const amount = moneyTransferStore.getObjects(
			null,
			"http://schema.org/amount")
			;
		  amountFromMoneyTransfer = amount[0].id.split('"')[1];
		 
		  const object = moneyTransferStore.getObjects(
			null, 
			"http://schema.org/object"
			);
		  objectFromMoneyTransfer = object[0].id;	  
	  }
	  catch (e) {
		 console.log("Not a money transfer!");
	  }
	  if (resourceURLFromMoneyTransfer !== undefined && amountFromMoneyTransfer !== undefined && objectFromMoneyTransfer !== undefined ) {
		 return {resourceURLFromMoneyTransfer, amountFromMoneyTransfer, objectFromMoneyTransfer};  
	  }	
	  else {
		  return null;
	  }
